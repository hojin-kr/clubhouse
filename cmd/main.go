...
func (s *server) GetMyJoins(ctx context.Context, in *pb.JoinRequest) (*pb.JoinReply, error) {
	...
	cursorStr := in.Cursor
	const pageSize = 100
	query := datastore.NewQuery(getDatastoreKind("Join")).Filter("AccountId =", in.Join.GetAccountId()).Filter("Status =", StatusJoinDefault).Filter("Status =", StatusJoinAccept).Filter("Status =", StatusJoinReject).Filter("Start >", time.Now().Unix()).Order("Start").Limit(pageSize)
	if cursorStr != "" {
		cursor, err := datastore.DecodeCursor(cursorStr)
		if err != nil {
			log.Fatalf("Bad cursor %q: %v", cursorStr, err)
...
func (s *server) GetGameJoins(ctx context.Context, in *pb.JoinRequest) (*pb.JoinReply, error) {
	...
	cursorStr := in.Cursor
	const pageSize = 100
	query := datastore.NewQuery(getDatastoreKind("Join")).Filter("GameId =", in.Join.GetGameId()).Filter("Status =", StatusJoinDefault).Filter("Status =", StatusJoinAccept).Filter("Status =", StatusJoinReject).Order("Created").Limit(pageSize)
	if cursorStr != "" {
		cursor, err := datastore.DecodeCursor(cursorStr)
		if err != nil {
			log.Fatalf("Bad cursor %q: %v", cursorStr, err)
...
func setChatPush(ctx context.Context, gameID int64, accountID int64, message string) {
	if message != "" {
		// accept account all
		var game pb.Game
		ds.Get(ctx, datastore.IDKey(getDatastoreKind("Game"), gameID, nil), &game)
		var apnsTokens []string
...
func setAccountIdPush(ctx context.Context, id int64, title string, body string) {
	var profile pb.Profile
	var apnsTokens []string
	ds.Get(ctx, datastore.IDKey(getDatastoreKind(getDatastoreKind("Profile")), id, nil), &profile)
	apnsTokens = append(apnsTokens, profile.ApnsToken)
